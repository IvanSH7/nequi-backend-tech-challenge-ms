...
public class FifoListener {
    ...
    private Flux<Void> listenRetryRepeat() {
        return listen()
                .doOnError(e -> log.error("Error listening FIFO queue", e))
                .repeat();
    }
    ...
    private Flux<Void> listen() {
        return getMessages()
                .flatMap(message -> processor.apply(message)
                        ...
                        maxNumberOfMessages)
                .onErrorContinue((e, o) -> log.error("Error listening FIFO message", e));
    }
    ...
    private Flux<Message> getMessages() {
        return Mono.fromCallable(this::getReceiveMessageRequest)
                ...
                .doOnNext(response -> log.debug("{} received messages from FIFO sqs", response.messages().size()))
